...
function compareTuple(a,b){ return a<b?-1:a>b?1:0; }

class ToTSafetyKernel {
  constructor(){this.lastAccepted=new Map();this.evidence=[];}
  validate(event){
    const required=["event_id","node_id","coordinate","epoch","sequence","payload_hash","prev_hash"];
    for(const k of required) if(event[k]===undefined) return {ok:false,reason:"MISSING_"+k.toUpperCase()};
    ...
    const key=event.node_id+"@"+event.coordinate;
...
class CoordinateDirectory {
  ...
  register(record){
    if(!/^[1-9][0-9]*:[1-9][0-9]*$/.test(record.coordinate)) return {ok:false,reason:"INVALID_COORDINATE"};
    ...
    this.records.set(record.coordinate,structuredClone(record));
    return {ok:true,record};
  }
  resolve(coordinate){return this.records.get(coordinate)||null;}
  snapshot(){return [...this.records.values()].sort((a,b)=>compareTuple(a.coordinate,b.coordinate));}
}

class Layer2Reconciler {
  constructor(kernel,directory){this.kernel=kernel;this.directory=directory;this.state=new Map();}
  ingest(event){
    const safety=this.kernel.validate(event);
    if(!safety.ok) return {status:"REJECTED",reason:safety.reason};
    ...
    const current=this.state.get(event.coordinate);
    ...
    if(!current || [incoming.epoch,incoming.sequence,incoming.node_id].join("|") > [current.epoch,current.sequence,current.node_id].join("|")){
      this.state.set(event.coordinate,incoming);
      return {status:"APPLIED",reason:"CAUSALLY_NEWER",receipt:safety.receipt};
    }
    if(hash(current)!==hash(incoming)) return {status:"CONFLICT",reason:"DIVERGENT_SAME_ORDER"};
    return {status:"IGNORED",reason:"DUPLICATE"};
  }
  snapshot(){return [...this.state.entries()].sort((a,b)=>compareTuple(a[0],b[0])).map(([coordinate,value])=>({coordinate,...value}));}
}

function makeEvent({node_id,coordinate,epoch,sequence,payload,prev_hash="GENESIS"}){
  return {event_id:hash({node_id,coordinate,epoch,sequence,payload}).slice(0,24),node_id,coordinate,epoch,sequence,payload,prev_hash,payload_hash:hash({node_id,coordinate,epoch,sequence,payload})};
}

function runFalsification(){
  const k=new ToTSafetyKernel(),d=new CoordinateDirectory(),r=new Layer2Reconciler(k,d),checks=[];
  function check(name,fn,expected){let actual;try{actual=fn()}catch(e){actual="THREW:"+e.message} checks.push({name,expected,actual,pass:JSON.stringify(actual)===JSON.stringify(expected)});}
  ...
  check("replay",()=>r.ingest(makeEvent({node_id:"N1",coordinate:"1:1",epoch:1,sequence:1,payload:{value:"A"},prev_hash:"GENESIS"})).status,"REJECTED");
  ...
  const d1=makeEvent({node_id:"N4",coordinate:"4:1",epoch:1,sequence:1,payload:{value:"P"}}); r.ingest(d1);
  const d2=makeEvent({node_id:"N4",coordinate:"4:1",epoch:1,sequence:2,payload:{value:"Q"},prev_hash:d1.payload_hash}); r.ingest(d2);
  const divergent=makeEvent({node_id:"N4",coordinate:"4:1",epoch:1,sequence:2,payload:{value:"R"},prev_hash:d1.payload_hash});
  check("same_order_divergence",()=>r.ingest(divergent).status,"REJECTED");
  const passed=checks.filter(x=>x.pass).length;
  const report={schema:"kex.layer2.falsification.v1",tests:checks,passed,total:checks.length,all_passed:passed===checks.length,directory:d.snapshot(),state:r.snapshot()};
